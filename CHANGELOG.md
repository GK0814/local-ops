...
- 修复 Codex 从不同终端或重启方式重启同一 Windows 项目后，因监听命令行变化被总控台误报为外部端口占用的问题：已同步卡片现在优先校验命令行中的服务工作目录，完整命令指纹仅作兼容兜底；相邻工作目录、非 Codex 卡片或多监听者不会被自动关联。
- 修复 Windows 上已归属项目从总控台启动后、经 IDE 或终端外部重启被误报为端口占用的问题；不能通过项目目录或兼容指纹证明归属时仍保持端口占用提示。
