...
总控台是一个面向 macOS 与 Windows 的本地服务与批处理任务快速启动、运行监测工具。它把常用项目命令、长期服务和一次性批处理任务集中到本地网页中，并用 Python 3 标准库提供只绑定回环地址的后端；前端是无构建、无 CDN 的原生 HTML/CSS/JavaScript。
...
- 每 2 秒查看当前用户的本地监听服务、CPU、物理内存占比和运行时长。
...
- Windows 11 或 macOS 12 及更高版本。
- Python 3.12。运行时仅使用 Python 标准库。
- Windows 使用系统自带的 PowerShell、`Get-NetTCPConnection` 与 `Get-CimInstance`；macOS 使用 `ps`、`lsof`、`osascript` 等系统工具。
...
日常使用推荐在桌面创建一次「总控台」图标：

```powershell
.\tools\install_desktop_shortcut.ps1
```

双击该图标时，若已有健康的总控台实例会直接打开它；否则会在后台启动实例、等待健康检查成功后再打开浏览器，不会重复启动，也不会保留 PowerShell 窗口。`start.bat` 保留为显示终端输出的排错入口。脚本会检查 Python 3.12+。配置、图标和日志默认位于 `%LOCALAPPDATA%\总控台`，其中日志目录为 `%LOCALAPPDATA%\总控台\logs`。

Windows 使用 `netstat` 与 `Get-CimInstance` 获取端口和进程信息；启动台通过 PowerShell 启动命令，并使用 Windows 进程树安全停止受控应用。为避免对外部进程做不可靠的工作目录猜测，“认领已有服务”在 Windows 上只会在信息可验证时开放。Codex 同步的项目若从 IDE、终端或 Codex 外部重启，只要监听者同端口、同用户、唯一 PID，且命令行明确引用该服务工作目录，就会自动恢复为项目运行中；完整命令行指纹只作兼容兜底。缺少项目路径证据或多个监听者时仍明确提示端口被占用。

### Codex MCP 同步

`tools/local_ops_mcp.py` 是零依赖的 stdio MCP 服务。把它注册到 Codex 后，
Codex 可将当前工作区同步到启动台：先通过本地规则识别候选命令，再创建卡片。
同步**不会启动项目**。一个包含 `frontend` / `backend`（也支持 `client`、`web`、`ui`、`api`、`server`）的一层目录项目，会在同一项目组中生成多个可独立启停的服务子卡片；每张卡片各自保存命令、日志、健康状态和端口。项目组汇总运行数量与各子服务的实际监听端口，不会把多个端口伪装成一个端口。再次同步仅刷新 MCP 元数据，不会覆盖你在启动台里手工修改的命令、名称或图标。

手工添加或编辑长期服务时，可在「归属项目」中选择启动台已有项目组；该服务会显示在项目组内，但仍保留独立命令、端口、日志与启停控制。选择「不加入项目」即可恢复独立卡片；批处理任务不支持项目归属。

在 PowerShell 中（将路径替换为本机实际项目目录）注册一次：
...
新开一个 Codex 任务后可使用 `get_project_candidates` 查看候选，或使用
`sync_project` 会登记每个独立服务角色中优先级最高的候选；如需只登记某个候选，可传入 `candidateIndex`。总控台必须已运行；服务会在
`127.0.0.1:9600` 至 `9609` 中自动发现当前实例。
...
启动后程序只绑定 `127.0.0.1`，从 9600 起尝试端口，被占用则递增（最多 10 个），并自动打开浏览器。Windows 可使用 `.\start.ps1 -NoBrowser` 或 `.\start.ps1 -PreferredPort 9603`；命令行参数、环境变量（`CONSOLE_DATA_DIR` / `CONSOLE_LOG_DIR`）见下文“数据、隐私与备份”。
...
- **卡片**：大按钮启动/停止（任务是运行/中止）；右侧一排小按钮（复制链接/日志/诊断/重启/编辑/删除）常显，不用悬浮。运行中显示端口与时长；配置失效（目录/脚本丢失）会直接标出原因并禁用启动，点开「启动诊断」有修复建议。项目识别为前端的服务（也包括自定义 `npm` 脚本）启动时，总控台会等待该受控服务实际监听配置端口（最长 30 秒）；未填写端口时，会使用该受管进程的首个实际监听端口，再在默认浏览器的新标签页打开页面。
...
- **概览卡**：在线服务/后台应用/总 CPU/总物理内存占比（带最近一分钟负载曲线）/端口警告/最后更新。
...
运行数据与程序目录分离：Windows 默认放在 `%LOCALAPPDATA%\总控台`，macOS 默认放在用户资料库：
...
Windows 的访问控制由当前用户的 `%LOCALAPPDATA%` ACL 负责，不使用 POSIX `0700` / `0600` 权限位。

### 旧版数据首次迁移

如果新目标目录尚不存在，首次启动会将项目内旧 `data/config.json{,.bak}` 和 `data/icons/` 安全复制到当前平台的默认数据目录，将 `data/logs/` 复制到默认日志目录。迁移使用临时目录后原子落位，并且：
...
tools/local_ops_mcp.py     Codex 工作区同步 MCP（stdio，零依赖）
